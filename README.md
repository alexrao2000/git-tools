...
| `git pr <n>` | Check out the branch for GitHub PR `#n`, no flags: switch to it if it's already local, otherwise fetch it first (falls back to `refs/pull/<n>/head` for merged/closed PRs). Branches are stamped with their PR number, so a repeat `git pr <n>` resolves **offline** with no network; `gh` is used only to resolve/fetch a branch you don't already have. |
| `git prs` | Print one link per line for every open PR you've authored in the current repo (nothing else — easy to skim or pipe). Needs the GitHub CLI (`gh`). |

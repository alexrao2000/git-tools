...
| `git done` | The bookend to `git pr`: switch back to the base (`origin/<default>`, usually main) and delete the branch you just left. Uses `git branch -D` on purpose — PR branches are usually squash-merged, which a safe `-d` wouldn't recognize as merged and would refuse; `-D` also drops genuinely unmerged work, so preview with `-n`. Guards against running on the base itself or a detached HEAD. `-n` dry-run, `-b NAME` base. |

...
| `git pr <n>` | Check out the branch for GitHub PR `#n`. If it's already local, just switch; otherwise fetch it (falls back to `refs/pull/<n>/head` for merged/closed PRs). Needs the GitHub CLI (`gh`). Stamps the branch with its PR number so `git here` can find it offline later. |
| `git here <n>` | The offline counterpart to `git pr`: switch to a PR's branch you **already have locally**, without fetching. For branches `git pr`/`git here` have seen it's fully offline (they're stamped with the PR number); otherwise it falls back to `gh pr view` once to resolve the name, then stamps it. Never fetches — if you don't have the branch it points you at `git pr <n>`. |

...
| `git pr [<n>]` | **List** your open PRs in this repo as `<url> -- <title>` (bare `git pr`); `--no-title`/`-nt` prints just the URLs, one per line (easy to pipe). **Or check one out**: `git pr <n\|branch\|url>` runs `gh pr checkout` (the argument can be a PR number, head branch name, or PR URL), creating the branch or fast-forwarding an existing one to the PR's latest — always up to date. Checkout falls back to `refs/pull/<n>/head` for merged/closed PRs whose branch was deleted, and — if GitHub is unreachable — to a local branch stamped with the PR number (switched to as-is, not updated). **Or merge them all**: `git pr --merge-all`/`-ma` merges every mergeable PR **you authored**, rebasing branches that need it and skipping anything that needs a human (listed at the end) — it never touches anyone else's PR, that's not this command's call to make, and never bypasses branch protection. Unlike the rest of this suite, `--merge-all` touches shared remote state (it merges PRs on GitHub); pass `-n`/`--dry-run` to preview the plan first without changing anything. It processes PRs in an order that minimizes conflicts (least-entangled first, by shared changed files — no LLM, just a conflict-adjacency count); for each PR, clean merges directly, needing-an-update rebases onto its base in a scratch worktree (never touches your current branch) and pushes, blocked/draft are always skipped. `--method merge\|squash\|rebase` overrides the merge method (default: the repo's own). Branches aren't auto-deleted — run `git sweep` after. Needs the GitHub CLI (`gh`). |
